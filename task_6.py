...
def withdraw_money(person, money):
    if person['money'] - money >= 0: # >= withdraw not only full amount
        person['money'] -= money
...
def process_user_choice(choice, person):
    if choice == 1: #check int not str
        print(check_account(person))
    elif choice == 2: #check int not str
        try:
            count = float(input('Сумма к снятию:'))
            print(withdraw_money(person, count))
        except ValueError:
            print('Укажите количество цифрами')
            count = float(input('Сумма к снятию:'))
            print(withdraw_money(person, count))
...
def start():
    ...
    try: #foolproof
        ...
        print('Неверный тип данных. Должны быть только цифры')
    ...
    if person and is_pin_valid(person, pin_code):
        ...
        print(f'Номер карты или пин код введены не верно!')
